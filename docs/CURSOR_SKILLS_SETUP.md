...
- プロンプトに **Skill の目的が書いてある語**を含める（例: `Step1` `MVP仮説` `Step2` `ストーリー` `Step3` `モック` `Step4` `実装` `Vercel` `デプロイ`）— `SKILL.md` の `description` がマッチしやすい  
...
| `course-mvp-step4-implement-host` | Step4 実装計画・`npm install`・実装・Vercel まで |
| `course-mvp-step2-scope-and-mock` | **非推奨**（旧一体型の案内のみ） |

参照: [`mvp/STEPS_MVP_FLOW.md`](mvp/STEPS_MVP_FLOW.md)
